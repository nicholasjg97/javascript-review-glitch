...
function endsWithVowel (str) {
    var vowelList = 'aeiouAEIOU';
    if (vowelList.indexOf(str[str.length - 1]) !== -1) {
        return true;
    } else {
        return false;
    }
...
function startsAndEndsWithVowel (str) {
    var vowelList = 'aeiouAEIOU';
    if (vowelList.indexOf(str[0]) !== -1 && vowelList.indexOf(str[str.length - 1]) !== -1) {
        return true;
    } else {
        return false;
    }
    // (startsWithVowel(str) && endsWithVowel(str));
}
...
function onlyPositiveEvens (seq) {
    var positiveEvens = [];
    for (var i = 0; i < seq.length; i++) {
        if (isPositive(seq[i]) && isEven(seq[i])) {
            positiveEvens.push(seq[i]);
        }
    }
...
function onlyPositiveOdds (seq) {
    var positiveOdds = [];
    for (var i = 0; i < seq.length; i++) {
        if (isPositive(seq[i]) && isOdd(seq[i])) {
            positiveOdds.push(seq[i]);
        }
    }
    return positiveOdds;
}
...
function onlyNegativeEvens (seq) {
    var negativeEvens = [];
    for (var i = 0; i < seq.length; i++) {
        if (isNegative(seq[i]) && isEven(seq[i])) {
            negativeEvens.push(seq[i]);
        }
    }
...
function onlyNegativeOdds (seq) {
    var negativeOdds = [];
    for (var i = 0; i < seq.length; i++) {
        if (isNegative(seq[i]) && isOdd(seq[i])) {
            negativeOdds.push(seq[i]);
        }
    }
    return negativeOdds;
}
...
function numberOfItemTypes (arr) {
    ...
    for (var i = 0; i < arr.items.length; i++) {
        if (uniqueItems.includes(arr.items[i].title)) {
            continue;
        }
        uniqueItems.push(arr.items[i].title);
    }
...
function totalNumberOfItems (arr) {
    var counter = 0;
    for (var i = 0; i < arr.items.length; i++) {
        counter += arr.items[i].quantity;
    }
    return counter;
}
...
function getAverageItemPrice (arr) {
    var sum = 0;
    for (var i = 0; i < arr.items.length; i++) {
        sum += arr.items[i].price;
    }
    return sum / arr.items.length;
}
...
function getAverageSpentPerItem (arr) {
    var average = 0;
    var quantity = 0;
    for (var i = 0; i < arr.items.length; i++) {
        quantity += arr.items[i].quantity;
        average += arr.items[i].price * arr.items[i].quantity;
    }
...
function mostSpentOnItem (arr) {
    ...
    for (var i = 0; i < arr.items.length; i++) {
        if ((arr.items[i].price * arr.items[i].quantity) > (mostSpent.price * mostSpent.quantity)) {
            mostSpent = arr.items[i];
        }
    }
    return mostSpent;
}
